...
import {FiArrowLeft, FiTag, FiTarget} from 'react-icons/fi';
import './styles.css';
...
const CreatePoint = () => {
    ...
    const[initialPosition,setinitialPosition] = useState<[number,number]>([-28.4311238,-48.869314]);
    ...
    useEffect(() => { //executada uma única vez
        navigator.geolocation.getCurrentPosition(position=>{
            const {latitude,longitude} = position.coords;
            setinitialPosition([latitude,longitude]);
        });
    } ,[])//caso colocar estador no vetor está função seria executa se eles mudassem de valor
        ...
        <div id="page-create-point" >
            ...
            <form onSubmit ={handleSubmit}>
                ...
                <fieldset>
                    ...
                    <Map center={initialPosition} zoom={15} onclick={handleMouseEvent} >
                        <TileLayer
                ...
                <fieldset>
                    ...
                    <ul className="items-grid">
                      {items.map((item) =>(
                          <li className={selectedItems.includes(item.id)?'selected':''} key={item.id} onClick={() => handleSelectedItems(item.id)}>
            ...
            <Link style={style} to="/" id="sucessModal" >
            <div  onClick={showScroll}>
             <FiCheckCircle /> <p>Cadastrado Com Sucesso!</p>
            </div>
            </Link>
        </div>
